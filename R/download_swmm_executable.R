...
if(!swmm_version %in% swmm_versions) {
  msg <- sprintf("Selected SWMM version '%s' is not available!\nPlease use one of: %s\n",
                 swmm_version,
...
url <- sprintf("%s/releases/download/v%s/swmm-solver-%s-%s.zip",
        repo,
        swmm_version,
        swmm_version,
        os)
...
if(!file.exists(swmm_exe)) {
  stop(sprintf("SWMM executable not available at assumed path: '%s'",
